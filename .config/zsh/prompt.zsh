#PROMPT="%B%{$fg[blue]%}[%{$fg[white]%}%\j%{$fg[red]%}@%{$fg[white]%}%\m%{$fg[blue]%}]%{$reset_color%}%  %(?:%{$fg_bold[green]%}➜ :%{$fg_bold[red]%}➜ )"
...
# Left Prompt
 PROMPT=' %B$(common_host)$(common_current_dir)$(common_bg_jobs)$(common_return_status)%b'
...
# Current directory
common_current_dir() {
  echo -n "%{$fg[$COMMON_COLORS_CURRENT_DIR]%}%2c "
}
