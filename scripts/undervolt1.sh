...
if [[ ( "$pattern"  && "`pattern_count`" -gt "0" ) || ( ! "$pattern" && "`line_count`" = "0" ) 
]]; then
 printf "${tc00f}> SKIPPED:$tcRst Already applied\n"
 return 1
fi
...
setting for that voltage will be $tc0f0$above_vids$tcRst VIDs above that to \"ensure\" 
stability.
...
printf "\nInstall required packages.\nWill use burnMMX (part of cpuburn package) to stress 
CPU."
...
if [ "$c" != "$count_tweak" ]; then
 printf "$tcf00> ERROR:$tcRst Number of frequencies ($c) and VIDs ($count_tweak) do not 
match!\n"
 exit 1
fi
check
...
replace it with the default VID.\n"
   let vid[c]=$def_vids
   vid_last="${vid[c]} "
  else
