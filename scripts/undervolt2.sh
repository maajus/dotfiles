...
{
	for i in 
/sys/devices/system/cpu/cpu*/cpufreq/{scaling_governor,cpuinfo_cur_freq,scaling_cur_freq,phc_vids}; 
do
...
if [[ -e "$crash_state" ]]; then
	...
	echo "Press return to continue from this point after a crash, or CTRL-C to delete the 
state file."
	read
...
for (( ; f<nb_freqs; f++ )); do
	...
	for (( ; cur_vid*vid_delta < vid_limit*vid_delta && pass2 == 0; cur_vid+=vid_delta )); 
do
		...
		# It looks like changing the VID also resets the frequency, so set frequency 
here
		...
		for (( ; count>0; count-- )); do
			...
			if (( (cur_freq < ${freq[f]} * (100 - wrong_freq_tolerance) / 100)
				|| (cur_freq > ${freq[f]} * (100 + wrong_freq_tolerance) / 
100) )); then
				echo ""
				echo "ERROR: Wrong frequency! (${cur_freq} instead of 
${freq[f]})"
				debug_info
				exit 1
			fi
	...
	for (( ; cur_vid*vid_delta <= vid_limit && cur_vid*vid_delta > 
(${vid[f]}+safety_vid_delta*vid_delta)*vid_delta; cur_vid-=vid_delta )); do
...
if [[ -e /etc/default/phc-intel ]]; then
	...
	echo "If your system has a working rc.local (probably not the case with systemd), you 
can add the following 3 lines to /etc/rc.local, before the final \"exit 0\":"
	echo ""
